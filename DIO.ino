...
#define ToggleBit(Reg,Pin) (Reg ^= (1<<Pin))//Changes the pin in the register if its 0 to 1 or 1 to 0
#define GetBit(Reg,Pin) (Reg&(1<<Pin))//function that gets the value on the pin of the register
#include "dio.h"

void Dio_SetPinDirection(unsigned char DDRX, unsigned char Pin,unsigned char Direction){//function that takes values for the DDR, pin in the DDR, either output or input
  switch (DDRX)//switch case function
  {
    
       case 'B'://registor B (DDRB)
    if (Direction == 1)//1==output
    {
      SetBit(DDRB,Pin);//sets the DDRB and Pin number
     
    }
    else{
      ClearBit(DDRB,Pin);    //clears Pin and DDRB 
